package com.androho.elektroeibauer.Pdf
...
import com.androho.elektroeibauer.Objects.WorktimeMain
...
class PDFCreator {
    ...
    private var dateMain: String = ""
    private var pathMain: String = ""
    private var customerMain: Customer? = null
    private var locationMain: String = ""
    private var doc: Document? = null
    private var drawableMain: Drawable? = null

    fun returnCreatedDocument(
        drawable: Drawable?,
        customer: Customer,
        date: String,
        arbeitsbeschreibung: String,
        worktimes: ArrayList<WorktimeMain>,
        path: String,
        count: Int,
        location: String
    ): Document {
...
// This adds the image to the page
    ...
    private fun tableMaterial() {
        ...
        if (count > 14) {
            ...
            doc!!.add(createHeader(drawableMain!!))


            doc!!.add(createAuftraggeber())


            countForNextPage = 1

            var countForLagerPDF = 1
            var newMat = ArrayList<CustomerMaterial>()
            for (mat in CustomerMaterial.customerMaterials) {

                if (countForLagerPDF > 13) {
                    newMat.add(mat)
            ...
            var tableNewPagePositions = Table(pointColumnWidths1)
            ...
            cell1.setBackgroundColor(com.itextpdf.kernel.color.Color.LIGHT_GRAY)
            cell1.setFontSize(7f)
            tableNewPagePositions.addCell(cell1)
            ...
            cell2.setHeight(10f)
            cell2.setFontSize(7f)
            tableNewPagePositions.addCell(cell2)
            ...
            cell4.setHeight(10f)
            cell4.setFontSize(7f)
            tableNewPagePositions.addCell(cell4)
            ...
            cell3.add("Material")
            cell3.setFontSize(7f)
            tableNewPagePositions.addCell(cell3)
            ...
            doc!!.add(tableNewPagePositions)


            var pointColumnWidths14 = floatArrayOf(50f, 50f, 50f, 350f)



            var tableNewPageMaterials = Table(pointColumnWidths14)

            var positionsNextPage = 14
            ...
            for (mat in newMat) {
                ...
                cell11.add(positionsNextPage.toString())
                cell11.setHeight(10f)
                cell11.setFontSize(7f)
                tableNewPageMaterials.addCell(cell11)

                var cell12 = Cell()
                cell12.setHeight(10f)
                cell12.add(mat.materialAmount)
                cell12.setFontSize(7f)
                tableNewPageMaterials.addCell(cell12)

                var cell13 = Cell()
                cell13.setHeight(10f)
                cell13.add(mat.materialUnit)
                cell13.setFontSize(7f)
                tableNewPageMaterials.addCell(cell13)
                ...
                cell14.setHeight(10f)
                cell14.setFontSize(7f)
                tableNewPageMaterials.addCell(cell14)
            ...
            //    count++
                ...
                var lastIndex = newMat.lastIndex

                if (lastIndex != countForNextPage - 2) {

                    if (countForNextPage.mod(26) == 1) {

                        doc!!.add(tableNewPageMaterials)
                        tableNewPageMaterials = Table(pointColumnWidths14)
            ...
            var pointColumnWidths13 = floatArrayOf(50f, 50f, 50f, 350f)
            var table1 = Table(pointColumnWidths13)
...
/*
    ...
    fun createAuftraggeber(): Table {
        ...
        if (customerMain!!.customerExpanded.clientName != "") {
            val cell3Auftrag = Cell()
            cell3Auftrag.setFontSize(11f)
            cell3Auftrag.add(customerMain!!.customerExpanded.clientName + " " + customerMain!!.customerExpanded.clientPreName + "\n" + customerMain!!.customerExpanded.clientStreetName + " " + customerMain!!.customerExpanded.clientStreetNumber + " \n" + customerMain!!.customerExpanded.clientPlz + " " + customerMain!!.customerExpanded.clientLocation)
            tableAuftrag.addCell(cell3Auftrag)
        } else {
            val cell3Auftrag = Cell()
            cell3Auftrag.setFontSize(11f)
            cell3Auftrag.add(customerMain!!.name + " " + customerMain!!.preName + "\n" + customerMain!!.streetName + " " + customerMain!!.streetNumber + " \n" + customerMain!!.plz + " " + customerMain!!.location)
            tableAuftrag.addCell(cell3Auftrag)
        }

        val cell4Auftrag = Cell()
        cell4Auftrag.setFontSize(11f)
        cell4Auftrag.add(customerMain!!.name + " " + customerMain!!.preName + "\n" + customerMain!!.streetName + " " + customerMain!!.streetNumber + " \n" + customerMain!!.plz + " " + customerMain!!.location)
        tableAuftrag.addCell(cell4Auftrag)

        val cell5Auftrag = Cell()
        cell5Auftrag.setFontSize(11f)
        if (customerMain!!.projectNumber != "") {
            cell5Auftrag.add("Projektnummer: " + customerMain!!.projectNumber)
        } else {
            cell5Auftrag.add("Projektnummer: ")
        }
        tableAuftrag.addCell(cell5Auftrag)
        ...
        val cell6Auftrag = Cell()
        cell6Auftrag.setFontSize(11f)
        cell6Auftrag.add("Datum: " + dateMain)
        tableAuftrag.addCell(cell6Auftrag)
